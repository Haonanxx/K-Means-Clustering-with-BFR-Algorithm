...
class KMeans:
    ...
    '''
    def __init__(self, K=8, seed=42, tol=0.5, max_iter=50):
        self.K = K
        self.seed = seed
        self.tol = tol
...
if __name__ == '__main__':
    ...
    for count, chunk in enumerate(data_chunks):
        ...
        if count == 0:
            ...
            y_label = KMeans(K=3*cluster_num, tol=0.03*math.sqrt(d)).fit(init_X)
            ...
            for label, indices in y_X_dict.items():
                ...
                if len(indices) < len(init_X)*0.018:
            ...
            cluster_res = dict()
            y_label = KMeans(K=cluster_num, tol=0.03*math.sqrt(d)).fit(inlier_points)
            ...
            if not RS:
                ...
                CS_dict = dict()
                y_label = KMeans(K=3*cluster_num, tol=0.03*math.sqrt(d)).fit(RS)
                print("and here?")
